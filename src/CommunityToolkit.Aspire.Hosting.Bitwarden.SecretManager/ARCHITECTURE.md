...
2. Authenticates with Bitwarden using the resolved credentials; looks up each managed secret's current value — by project ID from the AppHost cache when available, or by org-wide name search (`ListSecrets(organizationId)`) when no project ID is cached yet — and writes each found value to the deployment state via `IDeploymentStateManager`.
