...
Every secret has one deferred value provider, selected at construction. Parameter-backed managed secrets wrap their parameter input in a ReferenceExpression; resource-output secrets use the supplied expression. Reference-only secrets read the bound remote value directly, preserving null until resolution. Both IValueProvider overloads evaluate the same source. Parameter pre-sync eligibility is explicit and independent of the value-source type. The parameter adapter calls the inherited parameter getter so expression evaluation does not recurse through the secret provider.

- `BitwardenSecretManagerResource` declares a Bitwarden project and its configuration.
- `BitwardenSecretResource` declares either a managed secret (created or updated on every run, `IsManaged = true`) or a reference-only secret (read from an existing Bitwarden secret, `IsManaged = false`). Both modes inherit `ParameterResource` and are returned by `AddSecret` and `GetSecret` respectively as `IResourceBuilder<BitwardenSecretResource>`. `AddSecret` has three forms: a single-name form where the Aspire resource name and the Bitwarden secret name are the same, a two-name form (`name`, `remoteName`) where they differ, and an explicit-output form (`name`, `ReferenceExpression value`, optional `remoteName`) whose deferred source is authoritative. Explicit outputs bypass parameter input and upstream value pre-sync; their source is resolved before each write, including after an earlier successful reconciliation. `GetSecret` has three: those same two name-based forms, plus a GUID form (`name`, `secretId`) that locates a specific secret by its Bitwarden identifier. The GUID form is an escape hatch for when multiple secrets in the project share the same name — Bitwarden does not enforce remote name uniqueness. To inject the resolved secret value into a dependent resource, pass the secret builder directly to `WithEnvironment`. To inject the secret ID instead, call `.AsSecretId()` on the builder and pass the result to `WithEnvironment`.
