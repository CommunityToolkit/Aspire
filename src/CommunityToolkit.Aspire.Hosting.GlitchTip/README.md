...
Required missing or empty registrations, malformed source maps, rejected uploads, and upload timeouts fail deployment or opted-in local setup. Use `optional: true` only when absence is expected. It does not suppress upload errors. Identical artifacts already processed by the server are reused.

Upload success means GlitchTip accepted assembly with no missing chunks. GlitchTip stores uploaded chunks and queues processing before it returns this response. The integration does not wait for processing or symbolication. Project and DSN provisioning still block deployment on failure.

The shared platform owns alerts for later processing failures. GlitchTip can log these failures or record an internal assembly error without returning that error through its upload API. The integration does not automatically create an issue for these failures. Configure server monitoring separately. No retention or remote artifact cleanup policy is applied.
