...
public static class SeaweedFSBuilderExtensions
{
        ...
        [ResourceName] string name)
    {
        ...
        var accessKeyParameter = new ParameterResource("accessKey", _ => "admin");
        var secretKeyParameter = ParameterResourceBuilderExtensions.CreateDefaultPasswordParameter(builder, $"{name}-secretKey");

        var resource = new SeaweedFSContainerResource(name, accessKeyParameter, secretKeyParameter);
        ...
        {
            var isFilerEnabled = builderWithResource.Resource.Annotations.OfType<SeaweedFSFilerAnnotation>().Any();
            var isS3Enabled = builderWithResource.Resource.Annotations.OfType<SeaweedFSS3Annotation>().Any();
            var customConfig = builderWithResource.Resource.Annotations.OfType<SeaweedFSCustomS3ConfigAnnotation>().SingleOrDefault();
            ...
            else if (isS3Enabled)
            {
                ...
                var shellScript = "cat << EOF > /tmp/s3.json\n" +
        ...
        string configFilePath)
    {
        ...
        var containerPath = "/etc/seaweedfs/s3-custom.json";
