...
- If you're a Windows user and you don't have Perl, does it help you install well enough?  
-- The /scripts folder has alpha.ps1 and omega.ps1 both with a --validate mode
-- If you want to check first you can --validate alpha and see what you lack for the bulk of the libraries perl needs.
-- If you do NOT want to run someone else's script, does the readme cover what you need well enough?
...
- Seeing if I can get a worker service perl app to generate telemetry manually with traces.
...
- Determine how to get make and gcc on PATH in windows, CPAN doesn't include the Strawberry distributed c\bin\gcc.exe and CPANM does, but make still has an issue, so both need to be alerted to the user.
