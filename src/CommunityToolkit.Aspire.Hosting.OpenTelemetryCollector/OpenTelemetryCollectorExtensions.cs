        ...
        if (!settings.DisableHealthcheck)
        {
            ...
            resourceBuilder.WithEndpoint(targetPort: healthPort, name: "health", scheme: "http")
                ...
                .WithArgs(context =>
                {
                    ...
                    context.Args.Add("--feature-gates=confmap.enableMergeAppendOption");
                    context.Args.Add(ReferenceExpression.Create($"--config=yaml:extensions::health_check/aspire::endpoint: {healthEndpoint.Property(EndpointProperty.IPV4Host)}:{healthEndpoint.Property(EndpointProperty.TargetPort)}"));
                    context.Args.Add("--config=yaml:service::extensions: [ health_check/aspire ]");
                });
        }
