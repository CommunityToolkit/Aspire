...
public class EventStorePublicApiTests
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void WithLogShouldThrowWhenBuilderIsNull(bool useVolume)
    {
        IResourceBuilder<EventStoreResource> builder = null!;
        ...
        if (useVolume)
        {
            action = () => builder.WithLogVolume();
        }
        else
        {
            const string source = "/data";

            action = () => builder.WithLogBindMount(source);
        }
    ...
    [Fact]
    public void WithLogBindMountShouldThrowWhenSourceIsNull()
    {
        var builder = new DistributedApplicationBuilder([]);
        var eventstore = builder.AddEventStore("eventstore");
        ...
        var action = () => eventstore.WithLogBindMount(source);
    ...
    public void WithLogVolumeShouldAddMountAnnotation()
    {
        ...
        Assert.True(resource.TryGetLastAnnotation(out ContainerMountAnnotation? mountAnnotation));
        Assert.EndsWith("-log", mountAnnotation.Source);
        Assert.Equal("/var/log/eventstore", mountAnnotation.Target);
    }

    [Fact]
    public void WithNamedLogVolumeShouldAddMountAnnotation()
    {
        var builder = new DistributedApplicationBuilder([]);
        var eventstore = builder.AddEventStore("eventstore")
                                .WithLogVolume("eventstore-logs");
        using var app = builder.Build();
        ...
        Assert.True(resource.TryGetLastAnnotation(out ContainerMountAnnotation? mountAnnotation));
        Assert.Equal("eventstore-logs", mountAnnotation.Source);
        Assert.Equal("/var/log/eventstore", mountAnnotation.Target);
    }

    [Fact]
    public void WithLogBindMountShouldAddMountAnnotation()
    {
        var builder = new DistributedApplicationBuilder([]);
        var eventstore = builder.AddEventStore("eventstore")
                                .WithLogBindMount("./mylogs");
        using var app = builder.Build();
        ...
        Assert.True(resource.TryGetLastAnnotation(out ContainerMountAnnotation? mountAnnotation));
        Assert.EndsWith("mylogs", mountAnnotation.Source);
        Assert.Equal("/var/log/eventstore", mountAnnotation.Target);
    }
