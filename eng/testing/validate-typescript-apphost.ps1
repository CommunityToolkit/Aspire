...
"@ | Set-Content -Path $nugetConfigPath -NoNewline
    ...
    Invoke-CleanupStep -Description "Restore original aspire.config.json" -Failures $cleanupFailures -Action {
...
if ($cleanupFailures.Count -gt 0) {
    $cleanupMessage = "Cleanup failed:" + [Environment]::NewLine + (($cleanupFailures | ForEach-Object { " - $_" }) -join [Environment]::NewLine)
