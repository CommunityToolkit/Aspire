...
const javaProjectDirectory = path.join(
    appHostDirectory,
    "..",
    "CommunityToolkit.Aspire.Hosting.Java.Spring.Maven",
);
...
const _mavenGoalCommand: string = await mavenGoalApp.command();
const _mavenGoalWorkingDirectory: string =
    await mavenGoalApp.workingDirectory();
const _mavenGoalName: string = await mavenGoalApp.name();

const gradleBuildApp = await builder.addJavaApp(
    "java-gradle-build",
    javaProjectDirectory,
);
...
const gradleTaskApp = await builder.addJavaApp(
    "java-gradle-task",
    javaProjectDirectory,
);
await gradleTaskApp.withWrapperPath(isWindows ? "gradlew.bat" : "gradlew");
await gradleTaskApp.withGradleTask("bootRun", [
    "--args=--spring.main.banner-mode=off",
]);
