...
const stripeApiKey = await builder.addParameter("stripe-api-key", {
    value: "sk_test_123",
    secret: true,
});
const stripeApiKeyOverride = await builder.addParameter(
    "stripe-api-key-override",
    { value: "sk_test_override", secret: true },
);
...
const externalTarget = await builder.addExternalService(
    "external-target",
    "http://localhost:5082",
);
