...
const examplesRoot = path.resolve(appHostDirectory, "..");
...
const nxBlogWithOptions = await nxYarn.addApp("blog-monorepo-yarn", {
    appName: "blog-monorepo",
});
...
const nxPnpm = await builder.addNxApp("nx-pnpm", {
    workingDirectory: nxDemoRoot,
});
...
const turbo = await builder.addTurborepoApp("turborepo-demo", {
    workingDirectory: turborepoDemoRoot,
});
...
const turboYarn = await builder.addTurborepoApp("turborepo-yarn", {
    workingDirectory: turborepoDemoRoot,
});
...
const turboPnpm = await builder.addTurborepoApp("turborepo-pnpm", {
    workingDirectory: turborepoDemoRoot,
});
