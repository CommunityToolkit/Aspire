...
const _runtimePrimaryEndpoint =
    await runtimeOllamaResource.primaryEndpoint.get();
...
const _runtimeConnectionString =
    await runtimeOllamaResource.connectionStringExpression.get();
...
if (includeCompileOnlyScenarios) {
    const containerOllama = builder.addOllama("ollama-polyglot", {
        port: 11436,
    });
    ...
    const huggingFaceModel = containerOllama.addHuggingFaceModel(
        "llama-hf",
    ...
    await containerOllama.withOpenWebUI({
        ...
        configureContainer: async (openWebUI) => {
            ...
            const _openWebUiPrimaryEndpoint =
                await openWebUI.primaryEndpoint.get();
            ...
            const _openWebUiConnectionString =
                await openWebUI.connectionStringExpression.get();
    ...
    const _containerPrimaryEndpoint =
        await resolvedContainerOllama.primaryEndpoint.get();
    ...
    const _containerConnectionString =
        await resolvedContainerOllama.connectionStringExpression.get();
    ...
    const _autoNamedModelConnectionString =
        await resolvedAutoNamedModel.connectionStringExpression.get();
    ...
    const _namedModelConnectionString =
        await resolvedNamedModel.connectionStringExpression.get();
    ...
    const _huggingFaceModelName =
        await resolvedHuggingFaceModel.modelName.get();
    const _huggingFaceModelConnectionString =
        await resolvedHuggingFaceModel.connectionStringExpression.get();

    const localOllama = builder.addOllamaLocal("ollama-local", {
        targetPort: 11435,
    });
    ...
    const _localPrimaryEndpoint =
        await resolvedLocalOllama.primaryEndpoint.get();
    ...
    const _localConnectionString =
        await resolvedLocalOllama.connectionStringExpression.get();

    const resolvedLocalAutoNamedModel = await localAutoNamedModel;
    const _localAutoNamedModelParent =
        await resolvedLocalAutoNamedModel.parent.get();
    const _localAutoNamedModelName =
        await resolvedLocalAutoNamedModel.modelName.get();
    const _localAutoNamedModelConnectionString =
        await resolvedLocalAutoNamedModel.connectionStringExpression.get();
}
