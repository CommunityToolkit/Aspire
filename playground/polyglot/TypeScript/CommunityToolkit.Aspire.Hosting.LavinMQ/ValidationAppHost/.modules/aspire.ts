...
/** Handle to LavinMQContainerResource */
type LavinMQContainerResourceHandle = Handle<'CommunityToolkit.Aspire.Hosting.LavinMQ/Aspire.Hosting.ApplicationModel.LavinMQContainerResource'>;
...
export interface AddLavinMQOptions {
    amqpPort?: number;
...
export class DistributedApplicationBuilder {
    ...
    /** Adds a LavinMQ container resource */
    /** @internal */
    async _addLavinMQInternal(name: string, amqpPort?: number, managementPort?: number): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name };
        if (amqpPort !== undefined) rpcArgs.amqpPort = amqpPort;
        if (managementPort !== undefined) rpcArgs.managementPort = managementPort;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'CommunityToolkit.Aspire.Hosting.LavinMQ/addLavinMQ',
            rpcArgs
        );
        return new LavinMQContainerResource(result, this._client);
    }

    addLavinMQ(name: string, options?: AddLavinMQOptions): LavinMQContainerResourcePromise {
        const amqpPort = options?.amqpPort;
        const managementPort = options?.managementPort;
        return new LavinMQContainerResourcePromise(this._addLavinMQInternal(name, amqpPort, managementPort));
    }
...
export class DistributedApplicationBuilderPromise implements PromiseLike<DistributedApplicationBuilder> {
    ...
    /** Adds a LavinMQ container resource */
    addLavinMQ(name: string, options?: AddLavinMQOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.addLavinMQ(name, options)));
    }
...
export class LavinMQContainerResource extends ResourceBuilderBase<LavinMQContainerResourceHandle> {
    constructor(handle: LavinMQContainerResourceHandle, client: AspireClientRpc) {
        super(handle, client);
    }
    ...
    primaryEndpoint = {
        get: async (): Promise<EndpointReference> => {
            const handle = await this._client.invokeCapability<EndpointReferenceHandle>(
                'Aspire.Hosting.ApplicationModel/LavinMQContainerResource.primaryEndpoint',
                { context: this._handle }
            );
            return new EndpointReference(handle, this._client);
        },
    ...
    host = {
        get: async (): Promise<EndpointReferenceExpression> => {
            const handle = await this._client.invokeCapability<EndpointReferenceExpressionHandle>(
                'Aspire.Hosting.ApplicationModel/LavinMQContainerResource.host',
                { context: this._handle }
            );
    ...
    port = {
        get: async (): Promise<EndpointReferenceExpression> => {
            const handle = await this._client.invokeCapability<EndpointReferenceExpressionHandle>(
                'Aspire.Hosting.ApplicationModel/LavinMQContainerResource.port',
                { context: this._handle }
            );
            return new EndpointReferenceExpression(handle, this._client);
        },
    ...
    connectionStringExpression = {
        get: async (): Promise<ReferenceExpression> => {
            return await this._client.invokeCapability<ReferenceExpression>(
                'Aspire.Hosting.ApplicationModel/LavinMQContainerResource.connectionStringExpression',
                { context: this._handle }
            );
        },
    };
    ...
    uriExpression = {
        get: async (): Promise<ReferenceExpression> => {
            return await this._client.invokeCapability<ReferenceExpression>(
                'Aspire.Hosting.ApplicationModel/LavinMQContainerResource.uriExpression',
                { context: this._handle }
            );
        },
    };
    ...
    entrypoint = {
        get: async (): Promise<string> => {
            return await this._client.invokeCapability<string>(
                'Aspire.Hosting.ApplicationModel/LavinMQContainerResource.entrypoint',
                { context: this._handle }
            );
        },
        set: async (value: string): Promise<void> => {
            await this._client.invokeCapability<void>(
                'Aspire.Hosting.ApplicationModel/LavinMQContainerResource.setEntrypoint',
                { context: this._handle, value }
            );
        }
    };
    ...
    shellExecution = {
        get: async (): Promise<boolean> => {
            return await this._client.invokeCapability<boolean>(
                'Aspire.Hosting.ApplicationModel/LavinMQContainerResource.shellExecution',
                { context: this._handle }
            );
        },
        set: async (value: boolean): Promise<void> => {
            await this._client.invokeCapability<void>(
                'Aspire.Hosting.ApplicationModel/LavinMQContainerResource.setShellExecution',
                { context: this._handle, value }
            );
        }
    };
    ...
    name = {
        get: async (): Promise<string> => {
            return await this._client.invokeCapability<string>(
                'Aspire.Hosting.ApplicationModel/LavinMQContainerResource.name',
                { context: this._handle }
            );
        },
    };
    ...
    /** @internal */
    private async _withContainerRegistryInternal(registry: ResourceBuilderBase): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, registry };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withContainerRegistry',
            rpcArgs
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withContainerRegistryInternal(registry));
    }

    /** @internal */
    private async _withBindMountInternal(source: string, target: string, isReadOnly?: boolean): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source, target };
        if (isReadOnly !== undefined) rpcArgs.isReadOnly = isReadOnly;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withBindMount',
    ...
    /** Adds a bind mount */
    withBindMount(source: string, target: string, options?: WithBindMountOptions): LavinMQContainerResourcePromise {
        const isReadOnly = options?.isReadOnly;
        return new LavinMQContainerResourcePromise(this._withBindMountInternal(source, target, isReadOnly));
    }

    /** @internal */
    private async _withEntrypointInternal(entrypoint: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, entrypoint };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withEntrypoint',
            rpcArgs
    ...
    /** Sets the container entrypoint */
    withEntrypoint(entrypoint: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withEntrypointInternal(entrypoint));
    }

    /** @internal */
    private async _withImageTagInternal(tag: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, tag };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withImageTag',
            rpcArgs
    ...
    /** Sets the container image tag */
    withImageTag(tag: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withImageTagInternal(tag));
    }

    /** @internal */
    private async _withImageRegistryInternal(registry: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, registry };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withImageRegistry',
            rpcArgs
    ...
    /** Sets the container image registry */
    withImageRegistry(registry: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withImageRegistryInternal(registry));
    }

    /** @internal */
    private async _withImageInternal(image: string, tag?: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, image };
        if (tag !== undefined) rpcArgs.tag = tag;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withImage',
    ...
    /** Sets the container image */
    withImage(image: string, options?: WithImageOptions): LavinMQContainerResourcePromise {
        const tag = options?.tag;
        return new LavinMQContainerResourcePromise(this._withImageInternal(image, tag));
    }

    /** @internal */
    private async _withImageSHA256Internal(sha256: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, sha256 };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withImageSHA256',
            rpcArgs
    ...
    /** Sets the image SHA256 digest */
    withImageSHA256(sha256: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withImageSHA256Internal(sha256));
    }

    /** @internal */
    private async _withContainerRuntimeArgsInternal(args: string[]): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, args };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withContainerRuntimeArgs',
            rpcArgs
    ...
    /** Adds runtime arguments for the container */
    withContainerRuntimeArgs(args: string[]): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withContainerRuntimeArgsInternal(args));
    }

    /** @internal */
    private async _withLifetimeInternal(lifetime: ContainerLifetime): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, lifetime };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withLifetime',
            rpcArgs
    ...
    /** Sets the lifetime behavior of the container resource */
    withLifetime(lifetime: ContainerLifetime): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withLifetimeInternal(lifetime));
    }

    /** @internal */
    private async _withImagePullPolicyInternal(pullPolicy: ImagePullPolicy): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, pullPolicy };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withImagePullPolicy',
            rpcArgs
    ...
    /** Sets the container image pull policy */
    withImagePullPolicy(pullPolicy: ImagePullPolicy): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withImagePullPolicyInternal(pullPolicy));
    }

    /** @internal */
    private async _publishAsContainerInternal(): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/publishAsContainer',
            rpcArgs
    ...
    /** Configures the resource to be published as a container */
    publishAsContainer(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._publishAsContainerInternal());
    }

    /** @internal */
    private async _withDockerfileInternal(contextPath: string, dockerfilePath?: string, stage?: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, contextPath };
        if (dockerfilePath !== undefined) rpcArgs.dockerfilePath = dockerfilePath;
        if (stage !== undefined) rpcArgs.stage = stage;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
    ...
    /** Configures the resource to use a Dockerfile */
    withDockerfile(contextPath: string, options?: WithDockerfileOptions): LavinMQContainerResourcePromise {
        const dockerfilePath = options?.dockerfilePath;
        const stage = options?.stage;
        return new LavinMQContainerResourcePromise(this._withDockerfileInternal(contextPath, dockerfilePath, stage));
    }

    /** @internal */
    private async _withContainerNameInternal(name: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withContainerName',
            rpcArgs
    ...
    /** Sets the container name */
    withContainerName(name: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withContainerNameInternal(name));
    }

    /** @internal */
    private async _withBuildArgInternal(name: string, value: ParameterResource): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withBuildArg',
            rpcArgs
    ...
    /** Adds a build argument from a parameter resource */
    withBuildArg(name: string, value: ParameterResource): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withBuildArgInternal(name, value));
    }

    /** @internal */
    private async _withBuildSecretInternal(name: string, value: ParameterResource): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withBuildSecret',
            rpcArgs
    ...
    /** Adds a build secret from a parameter resource */
    withBuildSecret(name: string, value: ParameterResource): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withBuildSecretInternal(name, value));
    }

    /** @internal */
    private async _withEndpointProxySupportInternal(proxyEnabled: boolean): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, proxyEnabled };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withEndpointProxySupport',
            rpcArgs
    ...
    /** Configures endpoint proxy support */
    withEndpointProxySupport(proxyEnabled: boolean): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withEndpointProxySupportInternal(proxyEnabled));
    }

    /** @internal */
    private async _withDockerfileBaseImageInternal(buildImage?: string, runtimeImage?: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (buildImage !== undefined) rpcArgs.buildImage = buildImage;
        if (runtimeImage !== undefined) rpcArgs.runtimeImage = runtimeImage;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
    ...
    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): LavinMQContainerResourcePromise {
        const buildImage = options?.buildImage;
        const runtimeImage = options?.runtimeImage;
        return new LavinMQContainerResourcePromise(this._withDockerfileBaseImageInternal(buildImage, runtimeImage));
    }

    /** @internal */
    private async _withContainerNetworkAliasInternal(alias: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, alias };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withContainerNetworkAlias',
            rpcArgs
    ...
    /** Adds a network alias for the container */
    withContainerNetworkAlias(alias: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withContainerNetworkAliasInternal(alias));
    }

    /** @internal */
    private async _withMcpServerInternal(path?: string, endpointName?: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (path !== undefined) rpcArgs.path = path;
        if (endpointName !== undefined) rpcArgs.endpointName = endpointName;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
    ...
    /** Configures an MCP server endpoint on the resource */
    withMcpServer(options?: WithMcpServerOptions): LavinMQContainerResourcePromise {
        const path = options?.path;
        const endpointName = options?.endpointName;
        return new LavinMQContainerResourcePromise(this._withMcpServerInternal(path, endpointName));
    }

    /** @internal */
    private async _withOtlpExporterInternal(): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withOtlpExporter',
            rpcArgs
    ...
    /** Configures OTLP telemetry export */
    withOtlpExporter(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withOtlpExporterInternal());
    }

    /** @internal */
    private async _withOtlpExporterProtocolInternal(protocol: OtlpProtocol): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, protocol };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withOtlpExporterProtocol',
            rpcArgs
    ...
    /** Configures OTLP telemetry export with specific protocol */
    withOtlpExporterProtocol(protocol: OtlpProtocol): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withOtlpExporterProtocolInternal(protocol));
    }

    /** @internal */
    private async _publishAsConnectionStringInternal(): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/publishAsConnectionString',
            rpcArgs
    ...
    /** Publishes the resource as a connection string */
    publishAsConnectionString(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._publishAsConnectionStringInternal());
    }

    /** @internal */
    private async _withRequiredCommandInternal(command: string, helpLink?: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, command };
        if (helpLink !== undefined) rpcArgs.helpLink = helpLink;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withRequiredCommand',
    ...
    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): LavinMQContainerResourcePromise {
        const helpLink = options?.helpLink;
        return new LavinMQContainerResourcePromise(this._withRequiredCommandInternal(command, helpLink));
    }

    /** @internal */
    private async _withEnvironmentInternal(name: string, value: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withEnvironment',
            rpcArgs
    ...
    /** Sets an environment variable */
    withEnvironment(name: string, value: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withEnvironmentInternal(name, value));
    }

    /** @internal */
    private async _withEnvironmentExpressionInternal(name: string, value: ReferenceExpression): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withEnvironmentExpression',
            rpcArgs
    ...
    /** Adds an environment variable with a reference expression */
    withEnvironmentExpression(name: string, value: ReferenceExpression): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withEnvironmentExpressionInternal(name, value));
    }

    /** @internal */
    private async _withEnvironmentCallbackInternal(callback: (obj: EnvironmentCallbackContext) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as EnvironmentCallbackContextHandle;
            const obj = new EnvironmentCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Sets environment variables via callback */
    withEnvironmentCallback(callback: (obj: EnvironmentCallbackContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withEnvironmentCallbackInternal(callback));
    }

    /** @internal */
    private async _withEnvironmentCallbackAsyncInternal(callback: (arg: EnvironmentCallbackContext) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as EnvironmentCallbackContextHandle;
            const arg = new EnvironmentCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Sets environment variables via async callback */
    withEnvironmentCallbackAsync(callback: (arg: EnvironmentCallbackContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withEnvironmentCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withEnvironmentEndpointInternal(name: string, endpointReference: EndpointReference): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, endpointReference };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withEnvironmentEndpoint',
            rpcArgs
    ...
    /** Sets an environment variable from an endpoint reference */
    withEnvironmentEndpoint(name: string, endpointReference: EndpointReference): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withEnvironmentEndpointInternal(name, endpointReference));
    }

    /** @internal */
    private async _withEnvironmentParameterInternal(name: string, parameter: ParameterResource): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, parameter };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withEnvironmentParameter',
            rpcArgs
    ...
    /** Sets an environment variable from a parameter resource */
    withEnvironmentParameter(name: string, parameter: ParameterResource): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withEnvironmentParameterInternal(name, parameter));
    }

    /** @internal */
    private async _withEnvironmentConnectionStringInternal(envVarName: string, resource: ResourceBuilderBase): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, envVarName, resource };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withEnvironmentConnectionString',
            rpcArgs
    ...
    /** Sets an environment variable from a connection string resource */
    withEnvironmentConnectionString(envVarName: string, resource: ResourceBuilderBase): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withEnvironmentConnectionStringInternal(envVarName, resource));
    }

    /** @internal */
    private async _withConnectionPropertyInternal(name: string, value: ReferenceExpression): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withConnectionProperty',
            rpcArgs
    ...
    /** Adds a connection property with a reference expression */
    withConnectionProperty(name: string, value: ReferenceExpression): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withConnectionPropertyInternal(name, value));
    }

    /** @internal */
    private async _withConnectionPropertyValueInternal(name: string, value: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withConnectionPropertyValue',
            rpcArgs
    ...
    /** Adds a connection property with a string value */
    withConnectionPropertyValue(name: string, value: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withConnectionPropertyValueInternal(name, value));
    }

    /** @internal */
    private async _withArgsInternal(args: string[]): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, args };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withArgs',
            rpcArgs
    ...
    /** Adds arguments */
    withArgs(args: string[]): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withArgsInternal(args));
    }

    /** @internal */
    private async _withArgsCallbackInternal(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as CommandLineArgsCallbackContextHandle;
            const obj = new CommandLineArgsCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Sets command-line arguments via callback */
    withArgsCallback(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withArgsCallbackInternal(callback));
    }

    /** @internal */
    private async _withArgsCallbackAsyncInternal(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as CommandLineArgsCallbackContextHandle;
            const arg = new CommandLineArgsCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Sets command-line arguments via async callback */
    withArgsCallbackAsync(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withArgsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withReferenceInternal(source: ResourceBuilderBase, connectionName?: string, optional?: boolean, name?: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source };
        if (connectionName !== undefined) rpcArgs.connectionName = connectionName;
        if (optional !== undefined) rpcArgs.optional = optional;
        if (name !== undefined) rpcArgs.name = name;
    ...
    /** Adds a reference to another resource */
    withReference(source: ResourceBuilderBase, options?: WithReferenceOptions): LavinMQContainerResourcePromise {
        const connectionName = options?.connectionName;
        const optional = options?.optional;
        const name = options?.name;
        return new LavinMQContainerResourcePromise(this._withReferenceInternal(source, connectionName, optional, name));
    }

    /** @internal */
    private async _withReferenceUriInternal(name: string, uri: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, uri };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withReferenceUri',
            rpcArgs
    ...
    /** Adds a reference to a URI */
    withReferenceUri(name: string, uri: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withReferenceUriInternal(name, uri));
    }

    /** @internal */
    private async _withReferenceExternalServiceInternal(externalService: ExternalServiceResource): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, externalService };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withReferenceExternalService',
            rpcArgs
    ...
    /** Adds a reference to an external service */
    withReferenceExternalService(externalService: ExternalServiceResource): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withReferenceExternalServiceInternal(externalService));
    }

    /** @internal */
    private async _withReferenceEndpointInternal(endpointReference: EndpointReference): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, endpointReference };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withReferenceEndpoint',
            rpcArgs
    ...
    /** Adds a reference to an endpoint */
    withReferenceEndpoint(endpointReference: EndpointReference): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withReferenceEndpointInternal(endpointReference));
    }

    /** @internal */
    private async _withEndpointInternal(port?: number, targetPort?: number, scheme?: string, name?: string, env?: string, isProxied?: boolean, isExternal?: boolean, protocol?: ProtocolType): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (scheme !== undefined) rpcArgs.scheme = scheme;
    ...
    /** Adds a network endpoint */
    withEndpoint(options?: WithEndpointOptions): LavinMQContainerResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const scheme = options?.scheme;
        const name = options?.name;
        ...
        const isExternal = options?.isExternal;
        const protocol = options?.protocol;
        return new LavinMQContainerResourcePromise(this._withEndpointInternal(port, targetPort, scheme, name, env, isProxied, isExternal, protocol));
    }

    /** @internal */
    private async _withHttpEndpointInternal(port?: number, targetPort?: number, name?: string, env?: string, isProxied?: boolean): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (name !== undefined) rpcArgs.name = name;
    ...
    /** Adds an HTTP endpoint */
    withHttpEndpoint(options?: WithHttpEndpointOptions): LavinMQContainerResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const name = options?.name;
        const env = options?.env;
        const isProxied = options?.isProxied;
        return new LavinMQContainerResourcePromise(this._withHttpEndpointInternal(port, targetPort, name, env, isProxied));
    }

    /** @internal */
    private async _withHttpsEndpointInternal(port?: number, targetPort?: number, name?: string, env?: string, isProxied?: boolean): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (name !== undefined) rpcArgs.name = name;
    ...
    /** Adds an HTTPS endpoint */
    withHttpsEndpoint(options?: WithHttpsEndpointOptions): LavinMQContainerResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const name = options?.name;
        const env = options?.env;
        const isProxied = options?.isProxied;
        return new LavinMQContainerResourcePromise(this._withHttpsEndpointInternal(port, targetPort, name, env, isProxied));
    }

    /** @internal */
    private async _withExternalHttpEndpointsInternal(): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withExternalHttpEndpoints',
            rpcArgs
    ...
    /** Makes HTTP endpoints externally accessible */
    withExternalHttpEndpoints(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withExternalHttpEndpointsInternal());
    }
    ...
    /** @internal */
    private async _asHttp2ServiceInternal(): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/asHttp2Service',
            rpcArgs
    ...
    /** Configures resource for HTTP/2 */
    asHttp2Service(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._asHttp2ServiceInternal());
    }

    /** @internal */
    private async _withUrlsCallbackInternal(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as ResourceUrlsCallbackContextHandle;
            const obj = new ResourceUrlsCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withUrlsCallbackInternal(callback));
    }

    /** @internal */
    private async _withUrlsCallbackAsyncInternal(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ResourceUrlsCallbackContextHandle;
            const arg = new ResourceUrlsCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withUrlsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withUrlInternal(url: string, displayText?: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withUrl',
    ...
    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): LavinMQContainerResourcePromise {
        const displayText = options?.displayText;
        return new LavinMQContainerResourcePromise(this._withUrlInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlExpressionInternal(url: ReferenceExpression, displayText?: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withUrlExpression',
    ...
    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): LavinMQContainerResourcePromise {
        const displayText = options?.displayText;
        return new LavinMQContainerResourcePromise(this._withUrlExpressionInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlForEndpointInternal(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const obj = wrapIfHandle(objData) as ResourceUrlAnnotation;
            await callback(obj);
        });
    ...
    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withUrlForEndpointInternal(endpointName, callback));
    }

    /** @internal */
    private async _withUrlForEndpointFactoryInternal(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as EndpointReferenceHandle;
            const arg = new EndpointReference(argHandle, this._client);
            return await callback(arg);
    ...
    /** Adds a URL for a specific endpoint via factory callback */
    withUrlForEndpointFactory(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withUrlForEndpointFactoryInternal(endpointName, callback));
    }

    /** @internal */
    private async _excludeFromManifestInternal(): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/excludeFromManifest',
            rpcArgs
    ...
    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._excludeFromManifestInternal());
    }

    /** @internal */
    private async _waitForInternal(dependency: ResourceBuilderBase): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/waitFor',
            rpcArgs
    ...
    /** Waits for another resource to be ready */
    waitFor(dependency: ResourceBuilderBase): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._waitForInternal(dependency));
    }

    /** @internal */
    private async _waitForWithBehaviorInternal(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency, waitBehavior };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/waitForWithBehavior',
            rpcArgs
    ...
    /** Waits for another resource with specific behavior */
    waitForWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._waitForWithBehaviorInternal(dependency, waitBehavior));
    }

    /** @internal */
    private async _waitForStartInternal(dependency: ResourceBuilderBase): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/waitForStart',
            rpcArgs
    ...
    /** Waits for another resource to start */
    waitForStart(dependency: ResourceBuilderBase): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._waitForStartInternal(dependency));
    }

    /** @internal */
    private async _waitForStartWithBehaviorInternal(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency, waitBehavior };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/waitForStartWithBehavior',
            rpcArgs
    ...
    /** Waits for another resource to start with specific behavior */
    waitForStartWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._waitForStartWithBehaviorInternal(dependency, waitBehavior));
    }

    /** @internal */
    private async _withExplicitStartInternal(): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withExplicitStart',
            rpcArgs
    ...
    /** Prevents resource from starting automatically */
    withExplicitStart(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withExplicitStartInternal());
    }

    /** @internal */
    private async _waitForCompletionInternal(dependency: ResourceBuilderBase, exitCode?: number): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        if (exitCode !== undefined) rpcArgs.exitCode = exitCode;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/waitForCompletion',
    ...
    /** Waits for resource completion */
    waitForCompletion(dependency: ResourceBuilderBase, options?: WaitForCompletionOptions): LavinMQContainerResourcePromise {
        const exitCode = options?.exitCode;
        return new LavinMQContainerResourcePromise(this._waitForCompletionInternal(dependency, exitCode));
    }

    /** @internal */
    private async _withHealthCheckInternal(key: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, key };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withHealthCheck',
            rpcArgs
    ...
    /** Adds a health check by key */
    withHealthCheck(key: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withHealthCheckInternal(key));
    }

    /** @internal */
    private async _withHttpHealthCheckInternal(path?: string, statusCode?: number, endpointName?: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (path !== undefined) rpcArgs.path = path;
        if (statusCode !== undefined) rpcArgs.statusCode = statusCode;
        if (endpointName !== undefined) rpcArgs.endpointName = endpointName;
    ...
    /** Adds an HTTP health check */
    withHttpHealthCheck(options?: WithHttpHealthCheckOptions): LavinMQContainerResourcePromise {
        const path = options?.path;
        const statusCode = options?.statusCode;
        const endpointName = options?.endpointName;
        return new LavinMQContainerResourcePromise(this._withHttpHealthCheckInternal(path, statusCode, endpointName));
    }

    /** @internal */
    private async _withCommandInternal(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, commandOptions?: CommandOptions): Promise<LavinMQContainerResource> {
        const executeCommandId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ExecuteCommandContextHandle;
            const arg = new ExecuteCommandContext(argHandle, this._client);
            return await executeCommand(arg);
    ...
    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): LavinMQContainerResourcePromise {
        const commandOptions = options?.commandOptions;
        return new LavinMQContainerResourcePromise(this._withCommandInternal(name, displayName, executeCommand, commandOptions));
    }

    /** @internal */
    private async _withDeveloperCertificateTrustInternal(trust: boolean): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, trust };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withDeveloperCertificateTrust',
            rpcArgs
    ...
    /** Configures developer certificate trust */
    withDeveloperCertificateTrust(trust: boolean): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withDeveloperCertificateTrustInternal(trust));
    }

    /** @internal */
    private async _withCertificateTrustScopeInternal(scope: CertificateTrustScope): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, scope };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withCertificateTrustScope',
            rpcArgs
    ...
    /** Sets the certificate trust scope */
    withCertificateTrustScope(scope: CertificateTrustScope): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withCertificateTrustScopeInternal(scope));
    }

    /** @internal */
    private async _withHttpsDeveloperCertificateInternal(password?: ParameterResource): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (password !== undefined) rpcArgs.password = password;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withHttpsDeveloperCertificate',
    ...
    /** Configures HTTPS with a developer certificate */
    withHttpsDeveloperCertificate(options?: WithHttpsDeveloperCertificateOptions): LavinMQContainerResourcePromise {
        const password = options?.password;
        return new LavinMQContainerResourcePromise(this._withHttpsDeveloperCertificateInternal(password));
    }

    /** @internal */
    private async _withoutHttpsCertificateInternal(): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withoutHttpsCertificate',
            rpcArgs
    ...
    /** Removes HTTPS certificate configuration */
    withoutHttpsCertificate(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withoutHttpsCertificateInternal());
    }

    /** @internal */
    private async _withParentRelationshipInternal(parent: ResourceBuilderBase): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, parent };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withParentRelationship',
            rpcArgs
    ...
    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withParentRelationshipInternal(parent));
    }

    /** @internal */
    private async _withChildRelationshipInternal(child: ResourceBuilderBase): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, child };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withChildRelationship',
            rpcArgs
    ...
    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withChildRelationshipInternal(child));
    }

    /** @internal */
    private async _withIconNameInternal(iconName: string, iconVariant?: IconVariant): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, iconName };
        if (iconVariant !== undefined) rpcArgs.iconVariant = iconVariant;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withIconName',
    ...
    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): LavinMQContainerResourcePromise {
        const iconVariant = options?.iconVariant;
        return new LavinMQContainerResourcePromise(this._withIconNameInternal(iconName, iconVariant));
    }

    /** @internal */
    private async _withHttpProbeInternal(probeType: ProbeType, path?: string, initialDelaySeconds?: number, periodSeconds?: number, timeoutSeconds?: number, failureThreshold?: number, successThreshold?: number, endpointName?: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, probeType };
        if (path !== undefined) rpcArgs.path = path;
        if (initialDelaySeconds !== undefined) rpcArgs.initialDelaySeconds = initialDelaySeconds;
        if (periodSeconds !== undefined) rpcArgs.periodSeconds = periodSeconds;
    ...
    /** Adds an HTTP health probe to the resource */
    withHttpProbe(probeType: ProbeType, options?: WithHttpProbeOptions): LavinMQContainerResourcePromise {
        const path = options?.path;
        const initialDelaySeconds = options?.initialDelaySeconds;
        const periodSeconds = options?.periodSeconds;
        const timeoutSeconds = options?.timeoutSeconds;
        ...
        const successThreshold = options?.successThreshold;
        const endpointName = options?.endpointName;
        return new LavinMQContainerResourcePromise(this._withHttpProbeInternal(probeType, path, initialDelaySeconds, periodSeconds, timeoutSeconds, failureThreshold, successThreshold, endpointName));
    }

    /** @internal */
    private async _excludeFromMcpInternal(): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/excludeFromMcp',
            rpcArgs
    ...
    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._excludeFromMcpInternal());
    }

    /** @internal */
    private async _withRemoteImageNameInternal(remoteImageName: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageName };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withRemoteImageName',
            rpcArgs
    ...
    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withRemoteImageNameInternal(remoteImageName));
    }

    /** @internal */
    private async _withRemoteImageTagInternal(remoteImageTag: string): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageTag };
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'Aspire.Hosting/withRemoteImageTag',
            rpcArgs
    ...
    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withRemoteImageTagInternal(remoteImageTag));
    }

    /** @internal */
    private async _withPipelineStepFactoryInternal(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, dependsOn?: string[], requiredBy?: string[], tags?: string[], description?: string): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineStepContextHandle;
            const arg = new PipelineStepContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): LavinMQContainerResourcePromise {
        const dependsOn = options?.dependsOn;
        const requiredBy = options?.requiredBy;
        const tags = options?.tags;
        const description = options?.description;
        return new LavinMQContainerResourcePromise(this._withPipelineStepFactoryInternal(stepName, callback, dependsOn, requiredBy, tags, description));
    }

    /** @internal */
    private async _withPipelineConfigurationAsyncInternal(callback: (arg: PipelineConfigurationContext) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineConfigurationContextHandle;
            const arg = new PipelineConfigurationContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withPipelineConfigurationAsyncInternal(callback));
    }

    /** @internal */
    private async _withPipelineConfigurationInternal(callback: (obj: PipelineConfigurationContext) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as PipelineConfigurationContextHandle;
            const obj = new PipelineConfigurationContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._withPipelineConfigurationInternal(callback));
    }

    /** @internal */
    private async _withVolumeInternal(target: string, name?: string, isReadOnly?: boolean): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { resource: this._handle, target };
        if (name !== undefined) rpcArgs.name = name;
        if (isReadOnly !== undefined) rpcArgs.isReadOnly = isReadOnly;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
    ...
    /** Adds a volume */
    withVolume(target: string, options?: WithVolumeOptions): LavinMQContainerResourcePromise {
        const name = options?.name;
        const isReadOnly = options?.isReadOnly;
        return new LavinMQContainerResourcePromise(this._withVolumeInternal(target, name, isReadOnly));
    }
    ...
    /** @internal */
    private async _onBeforeResourceStartedInternal(callback: (arg: BeforeResourceStartedEvent) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as BeforeResourceStartedEventHandle;
            const arg = new BeforeResourceStartedEvent(argHandle, this._client);
            await callback(arg);
    ...
    /** Subscribes to the BeforeResourceStarted event */
    onBeforeResourceStarted(callback: (arg: BeforeResourceStartedEvent) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._onBeforeResourceStartedInternal(callback));
    }

    /** @internal */
    private async _onResourceStoppedInternal(callback: (arg: ResourceStoppedEvent) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ResourceStoppedEventHandle;
            const arg = new ResourceStoppedEvent(argHandle, this._client);
            await callback(arg);
    ...
    /** Subscribes to the ResourceStopped event */
    onResourceStopped(callback: (arg: ResourceStoppedEvent) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._onResourceStoppedInternal(callback));
    }

    /** @internal */
    private async _onConnectionStringAvailableInternal(callback: (arg: ConnectionStringAvailableEvent) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ConnectionStringAvailableEventHandle;
            const arg = new ConnectionStringAvailableEvent(argHandle, this._client);
            await callback(arg);
    ...
    /** Subscribes to the ConnectionStringAvailable event */
    onConnectionStringAvailable(callback: (arg: ConnectionStringAvailableEvent) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._onConnectionStringAvailableInternal(callback));
    }

    /** @internal */
    private async _onInitializeResourceInternal(callback: (arg: InitializeResourceEvent) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as InitializeResourceEventHandle;
            const arg = new InitializeResourceEvent(argHandle, this._client);
            await callback(arg);
    ...
    /** Subscribes to the InitializeResource event */
    onInitializeResource(callback: (arg: InitializeResourceEvent) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._onInitializeResourceInternal(callback));
    }

    /** @internal */
    private async _onResourceEndpointsAllocatedInternal(callback: (arg: ResourceEndpointsAllocatedEvent) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ResourceEndpointsAllocatedEventHandle;
            const arg = new ResourceEndpointsAllocatedEvent(argHandle, this._client);
            await callback(arg);
    ...
    /** Subscribes to the ResourceEndpointsAllocated event */
    onResourceEndpointsAllocated(callback: (arg: ResourceEndpointsAllocatedEvent) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._onResourceEndpointsAllocatedInternal(callback));
    }

    /** @internal */
    private async _onResourceReadyInternal(callback: (arg: ResourceReadyEvent) => Promise<void>): Promise<LavinMQContainerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ResourceReadyEventHandle;
            const arg = new ResourceReadyEvent(argHandle, this._client);
            await callback(arg);
    ...
    /** Subscribes to the ResourceReady event */
    onResourceReady(callback: (arg: ResourceReadyEvent) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._onResourceReadyInternal(callback));
    }

    /** @internal */
    private async _withDataVolumeInternal(name: string, isReadOnly?: boolean): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name };
        if (isReadOnly !== undefined) rpcArgs.isReadOnly = isReadOnly;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'CommunityToolkit.Aspire.Hosting.LavinMQ/withDataVolume',
            rpcArgs
        );
        return new LavinMQContainerResource(result, this._client);
    }

    /** Adds a data volume to a LavinMQ container resource */
    withDataVolume(name: string, options?: WithDataVolumeOptions): LavinMQContainerResourcePromise {
        const isReadOnly = options?.isReadOnly;
        return new LavinMQContainerResourcePromise(this._withDataVolumeInternal(name, isReadOnly));
    }

    /** @internal */
    private async _withDataBindMountInternal(source: string, isReadOnly?: boolean): Promise<LavinMQContainerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source };
        if (isReadOnly !== undefined) rpcArgs.isReadOnly = isReadOnly;
        const result = await this._client.invokeCapability<LavinMQContainerResourceHandle>(
            'CommunityToolkit.Aspire.Hosting.LavinMQ/withDataBindMount',
            rpcArgs
        );
        return new LavinMQContainerResource(result, this._client);
    }

    /** Adds a data bind mount to a LavinMQ container resource */
    withDataBindMount(source: string, options?: WithDataBindMountOptions): LavinMQContainerResourcePromise {
        const isReadOnly = options?.isReadOnly;
        return new LavinMQContainerResourcePromise(this._withDataBindMountInternal(source, isReadOnly));
    }

}

/**
 * Thenable wrapper for LavinMQContainerResource that enables fluent chaining.
 * @example
 * await builder.addSomething().withX().withY();
 */
export class LavinMQContainerResourcePromise implements PromiseLike<LavinMQContainerResource> {
    constructor(private _promise: Promise<LavinMQContainerResource>) {}

    then<TResult1 = LavinMQContainerResource, TResult2 = never>(
        onfulfilled?: ((value: LavinMQContainerResource) => TResult1 | PromiseLike<TResult1>) | null,
        onrejected?: ((reason: unknown) => TResult2 | PromiseLike<TResult2>) | null
    ): PromiseLike<TResult1 | TResult2> {
        return this._promise.then(onfulfilled, onrejected);
    }
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withContainerRegistry(registry)));
    }

    /** Adds a bind mount */
    withBindMount(source: string, target: string, options?: WithBindMountOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withBindMount(source, target, options)));
    }

    /** Sets the container entrypoint */
    withEntrypoint(entrypoint: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withEntrypoint(entrypoint)));
    }

    /** Sets the container image tag */
    withImageTag(tag: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withImageTag(tag)));
    }

    /** Sets the container image registry */
    withImageRegistry(registry: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withImageRegistry(registry)));
    }

    /** Sets the container image */
    withImage(image: string, options?: WithImageOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withImage(image, options)));
    }

    /** Sets the image SHA256 digest */
    withImageSHA256(sha256: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withImageSHA256(sha256)));
    }

    /** Adds runtime arguments for the container */
    withContainerRuntimeArgs(args: string[]): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withContainerRuntimeArgs(args)));
    }

    /** Sets the lifetime behavior of the container resource */
    withLifetime(lifetime: ContainerLifetime): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withLifetime(lifetime)));
    }

    /** Sets the container image pull policy */
    withImagePullPolicy(pullPolicy: ImagePullPolicy): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withImagePullPolicy(pullPolicy)));
    }

    /** Configures the resource to be published as a container */
    publishAsContainer(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.publishAsContainer()));
    }

    /** Configures the resource to use a Dockerfile */
    withDockerfile(contextPath: string, options?: WithDockerfileOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withDockerfile(contextPath, options)));
    }

    /** Sets the container name */
    withContainerName(name: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withContainerName(name)));
    }

    /** Adds a build argument from a parameter resource */
    withBuildArg(name: string, value: ParameterResource): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withBuildArg(name, value)));
    }

    /** Adds a build secret from a parameter resource */
    withBuildSecret(name: string, value: ParameterResource): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withBuildSecret(name, value)));
    }

    /** Configures endpoint proxy support */
    withEndpointProxySupport(proxyEnabled: boolean): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withEndpointProxySupport(proxyEnabled)));
    }

    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withDockerfileBaseImage(options)));
    }

    /** Adds a network alias for the container */
    withContainerNetworkAlias(alias: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withContainerNetworkAlias(alias)));
    }

    /** Configures an MCP server endpoint on the resource */
    withMcpServer(options?: WithMcpServerOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withMcpServer(options)));
    }

    /** Configures OTLP telemetry export */
    withOtlpExporter(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withOtlpExporter()));
    }

    /** Configures OTLP telemetry export with specific protocol */
    withOtlpExporterProtocol(protocol: OtlpProtocol): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withOtlpExporterProtocol(protocol)));
    }

    /** Publishes the resource as a connection string */
    publishAsConnectionString(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.publishAsConnectionString()));
    }

    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withRequiredCommand(command, options)));
    }

    /** Sets an environment variable */
    withEnvironment(name: string, value: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withEnvironment(name, value)));
    }

    /** Adds an environment variable with a reference expression */
    withEnvironmentExpression(name: string, value: ReferenceExpression): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withEnvironmentExpression(name, value)));
    }

    /** Sets environment variables via callback */
    withEnvironmentCallback(callback: (obj: EnvironmentCallbackContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withEnvironmentCallback(callback)));
    }

    /** Sets environment variables via async callback */
    withEnvironmentCallbackAsync(callback: (arg: EnvironmentCallbackContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withEnvironmentCallbackAsync(callback)));
    }

    /** Sets an environment variable from an endpoint reference */
    withEnvironmentEndpoint(name: string, endpointReference: EndpointReference): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withEnvironmentEndpoint(name, endpointReference)));
    }

    /** Sets an environment variable from a parameter resource */
    withEnvironmentParameter(name: string, parameter: ParameterResource): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withEnvironmentParameter(name, parameter)));
    }

    /** Sets an environment variable from a connection string resource */
    withEnvironmentConnectionString(envVarName: string, resource: ResourceBuilderBase): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withEnvironmentConnectionString(envVarName, resource)));
    }

    /** Adds a connection property with a reference expression */
    withConnectionProperty(name: string, value: ReferenceExpression): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withConnectionProperty(name, value)));
    }

    /** Adds a connection property with a string value */
    withConnectionPropertyValue(name: string, value: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withConnectionPropertyValue(name, value)));
    }

    /** Adds arguments */
    withArgs(args: string[]): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withArgs(args)));
    }

    /** Sets command-line arguments via callback */
    withArgsCallback(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withArgsCallback(callback)));
    }

    /** Sets command-line arguments via async callback */
    withArgsCallbackAsync(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withArgsCallbackAsync(callback)));
    }

    /** Adds a reference to another resource */
    withReference(source: ResourceBuilderBase, options?: WithReferenceOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withReference(source, options)));
    }

    /** Adds a reference to a URI */
    withReferenceUri(name: string, uri: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withReferenceUri(name, uri)));
    }

    /** Adds a reference to an external service */
    withReferenceExternalService(externalService: ExternalServiceResource): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withReferenceExternalService(externalService)));
    }

    /** Adds a reference to an endpoint */
    withReferenceEndpoint(endpointReference: EndpointReference): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withReferenceEndpoint(endpointReference)));
    }

    /** Adds a network endpoint */
    withEndpoint(options?: WithEndpointOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withEndpoint(options)));
    }

    /** Adds an HTTP endpoint */
    withHttpEndpoint(options?: WithHttpEndpointOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withHttpEndpoint(options)));
    }

    /** Adds an HTTPS endpoint */
    withHttpsEndpoint(options?: WithHttpsEndpointOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withHttpsEndpoint(options)));
    }

    /** Makes HTTP endpoints externally accessible */
    withExternalHttpEndpoints(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withExternalHttpEndpoints()));
    }
    ...
    /** Configures resource for HTTP/2 */
    asHttp2Service(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.asHttp2Service()));
    }

    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withUrlsCallback(callback)));
    }

    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withUrlsCallbackAsync(callback)));
    }

    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withUrl(url, options)));
    }

    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withUrlExpression(url, options)));
    }

    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withUrlForEndpoint(endpointName, callback)));
    }

    /** Adds a URL for a specific endpoint via factory callback */
    withUrlForEndpointFactory(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withUrlForEndpointFactory(endpointName, callback)));
    }

    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.excludeFromManifest()));
    }

    /** Waits for another resource to be ready */
    waitFor(dependency: ResourceBuilderBase): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.waitFor(dependency)));
    }

    /** Waits for another resource with specific behavior */
    waitForWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.waitForWithBehavior(dependency, waitBehavior)));
    }

    /** Waits for another resource to start */
    waitForStart(dependency: ResourceBuilderBase): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.waitForStart(dependency)));
    }

    /** Waits for another resource to start with specific behavior */
    waitForStartWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.waitForStartWithBehavior(dependency, waitBehavior)));
    }

    /** Prevents resource from starting automatically */
    withExplicitStart(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withExplicitStart()));
    }

    /** Waits for resource completion */
    waitForCompletion(dependency: ResourceBuilderBase, options?: WaitForCompletionOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.waitForCompletion(dependency, options)));
    }

    /** Adds a health check by key */
    withHealthCheck(key: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withHealthCheck(key)));
    }

    /** Adds an HTTP health check */
    withHttpHealthCheck(options?: WithHttpHealthCheckOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withHttpHealthCheck(options)));
    }

    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withCommand(name, displayName, executeCommand, options)));
    }

    /** Configures developer certificate trust */
    withDeveloperCertificateTrust(trust: boolean): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withDeveloperCertificateTrust(trust)));
    }

    /** Sets the certificate trust scope */
    withCertificateTrustScope(scope: CertificateTrustScope): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withCertificateTrustScope(scope)));
    }

    /** Configures HTTPS with a developer certificate */
    withHttpsDeveloperCertificate(options?: WithHttpsDeveloperCertificateOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withHttpsDeveloperCertificate(options)));
    }

    /** Removes HTTPS certificate configuration */
    withoutHttpsCertificate(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withoutHttpsCertificate()));
    }

    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withParentRelationship(parent)));
    }

    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withChildRelationship(child)));
    }

    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withIconName(iconName, options)));
    }

    /** Adds an HTTP health probe to the resource */
    withHttpProbe(probeType: ProbeType, options?: WithHttpProbeOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withHttpProbe(probeType, options)));
    }

    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.excludeFromMcp()));
    }

    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withRemoteImageName(remoteImageName)));
    }

    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withRemoteImageTag(remoteImageTag)));
    }

    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withPipelineStepFactory(stepName, callback, options)));
    }

    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withPipelineConfigurationAsync(callback)));
    }

    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withPipelineConfiguration(callback)));
    }

    /** Adds a volume */
    withVolume(target: string, options?: WithVolumeOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withVolume(target, options)));
    }
    ...
    /** Subscribes to the BeforeResourceStarted event */
    onBeforeResourceStarted(callback: (arg: BeforeResourceStartedEvent) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.onBeforeResourceStarted(callback)));
    }

    /** Subscribes to the ResourceStopped event */
    onResourceStopped(callback: (arg: ResourceStoppedEvent) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.onResourceStopped(callback)));
    }

    /** Subscribes to the ConnectionStringAvailable event */
    onConnectionStringAvailable(callback: (arg: ConnectionStringAvailableEvent) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.onConnectionStringAvailable(callback)));
    }

    /** Subscribes to the InitializeResource event */
    onInitializeResource(callback: (arg: InitializeResourceEvent) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.onInitializeResource(callback)));
    }

    /** Subscribes to the ResourceEndpointsAllocated event */
    onResourceEndpointsAllocated(callback: (arg: ResourceEndpointsAllocatedEvent) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.onResourceEndpointsAllocated(callback)));
    }

    /** Subscribes to the ResourceReady event */
    onResourceReady(callback: (arg: ResourceReadyEvent) => Promise<void>): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.onResourceReady(callback)));
    }

    /** Adds a data volume to a LavinMQ container resource */
    withDataVolume(name: string, options?: WithDataVolumeOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withDataVolume(name, options)));
    }

    /** Adds a data bind mount to a LavinMQ container resource */
    withDataBindMount(source: string, options?: WithDataBindMountOptions): LavinMQContainerResourcePromise {
        return new LavinMQContainerResourcePromise(this._promise.then(obj => obj.withDataBindMount(source, options)));
    }
...
registerHandleWrapper('CommunityToolkit.Aspire.Hosting.LavinMQ/Aspire.Hosting.ApplicationModel.LavinMQContainerResource', (handle, client) => new LavinMQContainerResource(handle as LavinMQContainerResourceHandle, client));
