...
const validateSecondResource =
    process.env.RAVENDB_VALIDATE_SECOND_RESOURCE === "true";
...
const _databaseConnectionString =
    await ordersDatabaseResource.connectionStringExpression.get();
...
const _databaseParentConnectionString =
    await ordersDatabaseParent.connectionStringExpression.get();

if (validateSecondResource) {
    const ravenWithBindMounts = await builder.addRavenDB("ravendb-bind");
    const inventoryDatabase =
        await ravenWithBindMounts.addDatabase("inventory");
    ...
    const _defaultDatabaseName: string =
        await inventoryDatabaseResource.databaseName.get();
    const _defaultDatabaseConnectionString =
        await inventoryDatabaseResource.connectionStringExpression.get();
}
